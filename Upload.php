...
class Upload {
	...
	public function __construct($path, $max_size) {
		if(!is_dir($path) || !is_writable($path)) {
			throw new \Exception('$path is not valid folder.');
		}
		if (!is_numeric($max_size) && !($max_size > 0)) {
            throw new \Exception('$size is not valid number for size in bytes.');
        } else {
	...
	/*
	* Call this method immediately after instantiation of the Upload class
	*
	* return undefined
	*/
    ...
    /*
	* Function call inside upload method
	*
	* return boolean
	*/
    ...
    /*
	* Function call inside checkFile function
	*
	* return boolean
	*/
